...
const defaultMenu = [
  ...
  },
  {
    ...
    images: ["https://images.unsplash.com/photo-1571934811356-5cc061b6821f?auto=format&fit=crop&w=900&q=80"],
  ...
  },
  {
    ...
    available: true,
    prepTime: 1,
    image: "https://images.unsplash.com/photo-1564419439244-61f879dd0c0d?auto=format&fit=crop&w=900&q=80",
    images: ["https://images.unsplash.com/photo-1564419439244-61f879dd0c0d?auto=format&fit=crop&w=900&q=80"],
  ...
  },
  {
    ...
    images: ["https://images.unsplash.com/photo-1529006557810-274b9b2fc783?auto=format&fit=crop&w=900&q=80"],
...
function printableReportHTML({ brandName, reportDate, sales, orderCount, deliveredCount, items }) {
  ...
  return `
  <html dir="rtl" lang="ar"><head><meta charset="UTF-8" /><title>تقرير المبيعات اليومية</title>
  <style>body{font-family:Arial,sans-serif;padding:32px;color:#111;background:#fff}.head{border:1px solid #ddd;border-radius:18px;padding:20px;margin-bottom:20px}h1,h2,p{margin:0 0 10px}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:20px 0}.box{border:1px solid #ddd;border-radius:14px;padding:16px;background:#fafafa}table{width:100%;border-collapse:collapse;margin-top:16px}th,td{border:1px solid #ddd;padding:12px;text-align:right}th{background:#f5f5f5}</style>
  </head><body><div class="head"><h1>${brandName}</h1><p>تقرير المبيعات اليومية</p><p>التاريخ: ${reportDate}</p></div>
  <div class="grid"><div class="box"><strong>إجمالي المبيعات</strong><br/>${money(sales)}</div><div class="box"><strong>عدد الطلبات اليوم</strong><br/>${orderCount}</div><div class="box"><strong>الطلبات المسلّمة</strong><br/>${deliveredCount}</div></div>
  <h2>الأصناف الأكثر مبيعاً</h2><table><thead><tr><th>#</th><th>الصنف</th><th>الكمية</th><th>المبيعات</th></tr></thead><tbody>${rows || '<tr><td colspan="4">لا توجد بيانات اليوم</td></tr>'}</tbody></table></body></html>`;
}
...
export default function App() {
    ...
    const setupData = async () => {
      try {
        ...
        if (currentMenu.empty) {
          for (const item of defaultMenu) await addDoc(collection(db, "menu"), { ...item, createdAt: serverTimestamp() });
        }
  ...
  const adminUnlocked = roleSession?.admin === true;
  const renderAdminGate = mode === "admin" && !adminUnlocked;
  ...
  const logoutAdmin = () => {
    setRoleSession((prev) => ({ ...prev, admin: false }));
    setMode("customer");
  };
  ...
  return (
    <div style={{ ...styles.app, background: pageBg, color: textColor, direction: language === "ar" ? "rtl" : "ltr" }}>
      <div style={styles.container}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {[["customer", "العميل", <QrCode size={16} />], ["cashier", "الاستقبال", <LayoutDashboard size={16} />], ["kitchen", "المطبخ", <ChefHat size={16} />], ["pickup", "الاستلام", <Bell size={16} />], ["admin", "الإدارة", <Settings size={16} />]].map(([value, label, icon]) => (
                <button key={String(value)} onClick={() => tryOpenMode(String(value))} style={{ ...styles.buttonSecondary, background: mode === value ? primaryColor : "white", color: mode === value ? "white" : "#111111", display: "flex", alignItems: "center", gap: 8 }}>
                  {icon}{label}{value === "admin" ? adminUnlocked ? <ShieldCheck size={15} /> : <Lock size={15} /> : null}
                </button>
              ))}
            </div>
          </div>
        ...
        {renderAdminGate ? (
          <div style={{ ...styles.card, maxWidth: 560, margin: "0 auto 24px" }}>
            <SectionTitle icon={<Lock size={20} />} title="دخول الإدارة" sub="هذه الصفحة فقط محمية بكلمة مرور" />
            <div style={{ marginTop: 18 }}>
              <label style={{ display: "block", marginBottom: 8, color: "#64748b" }}>كلمة المرور</label>
              <input style={styles.input} type="password" value={gatePassword} onChange={(e) => setGatePassword(e.target.value)} placeholder="ادخل كلمة المرور" onKeyDown={(e) => { if (e.key === "Enter") submitGate(); }} />
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
                ...
                <button style={{ ...styles.buttonSecondary, flex: 1 }} onClick={() => { setMode("customer"); setGatePassword(""); setGateError(""); }}>رجوع للعميل</button>
              </div>
            </div>
          </div>
        ) : null}

        {!renderAdminGate && mode === "customer" && (
                    ...
                    return (
                      <div key={item.id} style={{ ...styles.menuCard, background: isDark ? "#111827" : "#fff", color: textColor }}>
                        <div style={{ position: "relative" }}>
                          <img src={images[0]} alt={item.name} style={{ ...styles.menuImage, height: isMobile ? 200 : 180 }} onClick={() => openGallery(item, 0)} />
                          {images.length > 1 ? <div style={{ position: "absolute", bottom: 10, left: 10, background: "rgba(0,0,0,0.62)", color: "white", borderRadius: 999, padding: "6px 10px", fontSize: 12 }}>{images.length} صور</div> : null}
                        </div>
                        ...
                        <div style={{ padding: 16 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}><div><div style={{ fontSize: isMobile ? 18 : 20, fontWeight: 800 }}>{item.name}</div><div style={{ color: mutedColor, marginTop: 6 }}>{item.category}</div></div><div style={{ fontWeight: 800, fontSize: 16, color: primaryColor, whiteSpace: "nowrap" }}>{money(item.price)}</div></div>
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 14, gap: 10 }}><div style={{ color: mutedColor, fontSize: 13 }}>{t.prepTime}: {item.prepTime} {t.minutes}</div><button style={{ ...styles.button, background: primaryColor, minWidth: 94 }} onClick={() => addToCart(item)}>{t.add}</button></div>
                        </div>
                      </div>
                    );
                  })}
        ...
        {!renderAdminGate && (mode === "cashier" || mode === "kitchen" || mode === "pickup") && (
          <div>
      ...
      {gallery ? (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.86)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 18 }} onClick={closeGallery}>
          <div style={{ position: "relative", width: "min(980px, 100%)", maxHeight: "92vh" }} onClick={(e) => e.stopPropagation()}>
            <button onClick={closeGallery} style={{ position: "absolute", top: -8, right: -8, zIndex: 2, ...styles.buttonSecondary, borderRadius: 999, padding: 10 }}><X size={20} /></button>
            <div style={{ color: "white", marginBottom: 12, fontWeight: 800, fontSize: 18, textAlign: "center" }}>{gallery.title}</div>
            <div style={{ position: "relative", background: "#111", borderRadius: 24, overflow: "hidden" }}>
              <img src={gallery.images[gallery.index]} alt={gallery.title} style={{ width: "100%", maxHeight: "78vh", objectFit: "contain", display: "block" }} />
              {gallery.images.length > 1 ? <>
                <button onClick={prevGallery} style={{ position: "absolute", top: "50%", right: 12, transform: "translateY(-50%)", ...styles.buttonSecondary, borderRadius: 999, padding: 10 }}><ChevronRight size={22} /></button>
                <button onClick={nextGallery} style={{ position: "absolute", top: "50%", left: 12, transform: "translateY(-50%)", ...styles.buttonSecondary, borderRadius: 999, padding: 10 }}><ChevronLeft size={22} /></button>
                <div style={{ position: "absolute", bottom: 12, left: "50%", transform: "translateX(-50%)", background: "rgba(0,0,0,0.6)", color: "white", padding: "6px 12px", borderRadius: 999, fontSize: 13 }}>{gallery.index + 1} / {gallery.images.length}</div>
              </> : null}
            </div>
          </div>
        </div>
