    ...
    <div style={styles.app}>
      <div style={styles.container}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 16, justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ flex: "1 1 420px" }}>
              <div style={{ display: "inline-flex", gap: 8, alignItems: "center", background: "rgba(255,255,255,0.1)", padding: "10px 14px", borderRadius: 999, fontSize: 14 }}>
                <Sparkles size={16} />
                نظام احترافي للفود ترك والكافيهات والمطاعم المتحركة
              </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 24 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              <div style={styles.card}>
                <SectionTitle icon={<Store size={20} />} title="صفحة العميل من QR" sub="هذه الصفحة يفتحها العميل من الباركود الموجود على الكوفي ترك أو الطاولة أو الكاونتر" />
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 20 }}>
                  ...
                  <div style={{ gridColumn: "1 / -1", display: "flex", justifyContent: "space-between", alignItems: "center", border: "1px solid #e5e7eb", background: "#fafaf9", borderRadius: 18, padding: 16 }}>
                    <div>
                      ...
                      <div style={{ color: "#64748b", fontSize: 14, marginTop: 4 }}>لاحقًا يتم ربطه مع واتساب أو SMS أو شاشة نداء</div>
                    </div>
              ...
              <div style={styles.card}>
                ...
                <div style={{ color: "#64748b", marginBottom: 16 }}>الأصناف المتاحة فقط تظهر للعميل، ويمكن التحكم بها من لوحة الإدارة</div>
            ...
            <div>
              <div style={{ ...styles.card, position: "sticky", top: 16 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  <button style={styles.button} onClick={placeOrder} disabled={!customerName || !phone || cart.length === 0}>إرسال الطلب إلى النظام الداخلي</button>
                  <div style={{ border: "1px solid #a7f3d0", background: "#ecfdf5", color: "#065f46", borderRadius: 18, padding: 16, lineHeight: 1.8 }}>
                    عند الإرسال ينتقل الطلب فورًا إلى شاشة الاستقبال داخل الكوفي أو الفود ترك، ومن هناك يتحول إلى المطبخ، ثم إلى شاشة الاستلام، ثم يتم التواصل مع العميل عند الجاهزية.
                  </div>
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            <div style={styles.card}>
              <SectionTitle
                ...
                sub={mode === "cashier" ? "هذه هي الشاشة التي تستقبل الطلبات من جوالات العملاء" : mode === "kitchen" ? "تعرض الطلبات المطلوب تحضيرها فقط" : "تعرض الطلبات الجاهزة للتسليم والتواصل"}
              />
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 20 }}>
                  ...
                  <div key={order.id} style={{ border: "1px solid #e5e7eb", borderRadius: 24, padding: 18, boxShadow: "0 4px 14px rgba(0,0,0,0.04)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                      <div>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 6, color: "#94a3b8", marginTop: 6, fontSize: 14 }}><Clock3 size={16} /> {order.createdAt}</div>
                      </div>
                    ...
                    <div style={{ border: "1px solid #e5e7eb", background: "#fafaf9", borderRadius: 18, padding: 16, marginTop: 16 }}>
                      {order.items.map((item) => (
                        <div key={`${order.id}-${item.id}`} style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                    ...
                    {order.notifyCustomer && order.status === "ready" ? (
                      <div style={{ border: "1px solid #a7f3d0", background: "#ecfdf5", color: "#065f46", borderRadius: 18, padding: 16, marginTop: 12, lineHeight: 1.8, display: "flex", gap: 8 }}>
                        <Bell size={16} style={{ marginTop: 4 }} />
                        <span>في النسخة الحقيقية: هنا يتم إرسال واتساب أو SMS تلقائيًا إلى {order.customerName} على الرقم {order.phone} بأن الطلب جاهز للاستلام.</span>
                      </div>
                    ) : null}
                  </div>
                ))}
                ...
                {menu.map((item) => (
                  <div key={item.id} style={{ border: "1px solid #e5e7eb", borderRadius: 24, padding: 16, display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                      ...
                      <button style={styles.buttonSecondary} onClick={() => toggleAvailability(item.id)}>{item.available ? "إيقاف" : "تفعيل"}</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div style={{ ...styles.card, background: "#0f0f0f", color: "white", marginTop: 24 }}>
          <div style={{ fontSize: 28, fontWeight: 800, marginBottom: 12 }}>البنية الحقيقية للمشروع بعد البرمجة</div>
          <div style={{ color: "rgba(255,255,255,0.8)", lineHeight: 2 }}>
            1) العميل يفتح صفحة QR من الجوال ويطلب. <br />
            2) الطلب يُحفظ في Firebase / قاعدة البيانات. <br />
            3) يظهر فورًا في شاشة الاستقبال داخل الكوفي أو الفود ترك. <br />
            4) يتحول إلى شاشة المطبخ عند بدء التحضير. <br />
            5) عند الجاهزية يظهر في شاشة الاستلام. <br />
            6) يتم إرسال إشعار للعميل عبر واتساب أو SMS أو شاشة نداء. <br />
            7) الإدارة تتحكم في المنيو، التوافر، المبيعات، والتشغيل اليومي.
          </div>
        </div>
      </div>
    </div>
